...
def prepare_data(role):
    ...
    df3 = df3[['repo', 'people', 'behavior_risk', 'pr_num', 'avg_response_time', 'avg_response_interval', 'review_rate']]
    output_path = f"{SUMMARY_DIR}/{role}_topsis_data.xls"
    df3.to_excel(output_path, index=False, header=True)



def topsis_maintainer(role, weights):
    file = f"{SUMMARY_DIR}/{role}_topsis_data.xls"
    df_origin = pd.read_excel(file)
    df_origin['avg_response_time'] = df_origin['avg_response_time'] / 1440
    df_origin['avg_response_interval'] = df_origin['avg_response_interval'] / 1440

    df = df_origin.copy()
    df["behavior_risk"] = df["behavior_risk"].apply(np.log1p)
    df["pr_num"] = df["pr_num"].apply(np.log1p)
    df["avg_response_time"] = df["avg_response_time"].apply(np.log1p)
    df["avg_response_interval"] = df["avg_response_interval"].apply(np.log1p)
...
if __name__ == '__main__':
    prepare_data("maintainer")

    weights = np.array([0.61178203, 0.0312935, 0.09860067, 0.09860067, 0.15972313])  # [behavior_risk, pr_num, avg_response_time, avg_response_interval, review_rate]

    topsis_maintainer("maintainer", weights)